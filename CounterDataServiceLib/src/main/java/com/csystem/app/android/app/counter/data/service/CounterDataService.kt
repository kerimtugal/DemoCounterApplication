package com.csystem.app.android.app.counter.data.service
...
class CounterDataService @Inject constructor(@ApplicationContext context: Context,
                                             @DateTimeFormatterTRInterceptor formatter: DateTimeFormatter) {
    private val mContext = context
    private val mDateTimeFormatter = formatter
    ...
    fun findAll(): List<List<String>> {
        try {
            BufferedReader(mContext.openFileInput(FILE_NAME).reader(StandardCharsets.UTF_8)).use {
                return it.readLines().map { it1 ->
                    it1.split(" ")
                }
            }
        } catch (_: FileNotFoundException) {
